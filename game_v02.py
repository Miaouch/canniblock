...
# music.play("voxel-revolution-by-kevin-macleod-from-filmmusic-io")
# music.set_volume(0.1)

for x in range (0, 800, 100):
    for y in range (0, 30 * 3, 30):
        super_brick = Actor("block_happy", anchor = ["left", "top"])
        super_brick.pos = [x, y * 2]
...
def draw(): 
    ...
    if ball.bottom > HEIGHT and all_second_balls == []:
        game_over.draw()
...
def update(dt):
    ...
    for balls_multiples in all_balls_multiples:                
        if player.colliderect(balls_multiples):
            # if second_ball in all_second_balls: ---------------pour avoir d'autre ball bleue en plus----------------
            #     for i in range(len(all_second_balls)):
            #         second_ball = Actor ("ball_blue")
